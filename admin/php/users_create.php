...
if (isset($_POST['register'])) {
   ...
   $roles = filter_input(INPUT_POST, 'users_role', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
   ...
   if ($stmtRegisterValidate->rowCount() == 1) {
      ...
      $sqlCreate = "INSERT INTO users_accounts (last_names, first_names, company, emails, passwords, roles, statuses) VALUES (?,?,?,?,?,?,?)";
      $stmtCreate = $con->prepare($sqlCreate);
      $stmtCreate->execute([$lastName, $firstName, $company, $email, $password, $roles, $status]);
